...
class RunbookRetriever:
    def __init__(self, runbooks_dir: Path | str | None = None) -> None:
        self.runbooks_dir = Path(runbooks_dir) if runbooks_dir else Path(__file__).resolve().parents[1] / "runbooks"
    ...
    def retrieve(
        ...
        try:
            ...
            results = matched_chunks[:top_k]

            logger.info("Runbook retrieval matched chunk_count=%s", len(results))
            for chunk in results:
                logger.info(
                    "Runbook chunk docId=%s section=%s score=%s",
                    chunk.docId,
                    chunk.section,
                    chunk.score,
                )
    ...
    def _with_score(self, chunk: RunbookChunk, query_keywords: set[str]) -> RunbookChunk:
        ...
        for keyword in query_keywords:
            if keyword in self._tokens_from_value(searchable_fields["keywords"]):
                score += 3.0
            if keyword in self._tokens_from_value(searchable_fields["title"]):
                score += 2.0
            if keyword in self._tokens_from_value(searchable_fields["section"]):
                score += 2.0
            if keyword in self._tokens_from_value(searchable_fields["content"]):
                score += 1.0
