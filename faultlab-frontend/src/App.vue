...
const scenarios = [
  ...
  },
  {
    ...
    name: '下游接口超时',
    alias: 'Downstream Timeout',
    description:
      '模拟当前服务调用下游接口时，下游响应变慢或超时，导致当前接口延迟升高、请求失败或进入 fallback。可通过合理超时、fallback、隔离下游线程池、熔断降级治理。',
    defaults: {
      requestCount: 100,
      concurrency: 20,
    ...
    fields: [
      { key: 'requestCount', label: '请求总数', type: 'number', min: 1 },
      { key: 'concurrency', label: '并发数', type: 'number', min: 1 },
      { key: 'downstreamDelayMs', label: '下游延迟 ms', type: 'number', min: 0 },
      { key: 'timeoutMs', label: '调用超时 ms', type: 'number', min: 1 },
      { key: 'timeoutRatio', label: '超时比例', type: 'number', min: 0, max: 1, step: 0.1 },
      { key: 'fallbackDelayMs', label: 'Fallback 延迟 ms', type: 'number', min: 0 },
      { key: 'enableFallback', label: '启用 fallback', type: 'boolean' },
    ],
  },
  {
    code: 'RETRY_STORM',
    ...
    alias: 'Retry Storm',
    description:
      '模拟下游失败后，上游进行多次重试，导致下游调用量被放大，形成重试风暴。可通过限制重试次数、指数退避、jitter、重试预算和熔断治理。',
    defaults: {
      requestCount: 100,
      concurrency: 20,
      failureRatio: 0.7,
    ...
    fields: [
      { key: 'requestCount', label: '请求总数', type: 'number', min: 1 },
      { key: 'concurrency', label: '并发数', type: 'number', min: 1 },
      { key: 'failureRatio', label: '失败比例', type: 'number', min: 0, max: 1, step: 0.1 },
      { key: 'maxRetries', label: '最大重试次数', type: 'number', min: 0 },
      { key: 'retryBackoffMs', label: '重试退避 ms', type: 'number', min: 0 },
      { key: 'enableRetryLimit', label: '启用重试限制', type: 'boolean' },
      { key: 'enableJitter', label: '启用 jitter', type: 'boolean' },
    ],
  },
  {
    code: 'CIRCUIT_BREAKER_OPEN',
    name: '熔断器打开',
    alias: 'Circuit Breaker Open',
    description:
      '模拟下游失败率或慢调用比例超过阈值后，熔断器打开，后续请求被快速拒绝或进入 fallback。可通过合理熔断阈值、半开探测、fallback 和下游隔离治理。',
    defaults: {
      requestCount: 100,
      failureRatio: 0.8,
      slowCallRatio: 0.5,
    ...
    fields: [
      { key: 'requestCount', label: '请求总数', type: 'number', min: 1 },
      { key: 'failureRatio', label: '失败比例', type: 'number', min: 0, max: 1, step: 0.1 },
      { key: 'slowCallRatio', label: '慢调用比例', type: 'number', min: 0, max: 1, step: 0.1 },
      { key: 'slidingWindowSize', label: '滑动窗口大小', type: 'number', min: 1 },
      { key: 'failureRateThreshold', label: '失败率阈值', type: 'number', min: 0, max: 1, step: 0.1 },
      { key: 'slowCallThresholdMs', label: '慢调用阈值 ms', type: 'number', min: 1 },
      { key: 'openDurationMs', label: '打开持续 ms', type: 'number', min: 0 },
      { key: 'enableFallback', label: '启用 fallback', type: 'boolean' },
    ],
  },
]
